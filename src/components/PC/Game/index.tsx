...
const BlackjackGame: React.FC = () => {
  ...
  const hit = async () => {
    ...
    if (handValue >= 21) {
      // 大于21点庄家赢，等于21 玩家赢
      const is21 = handValue === 21
      newMessage = is21 ? MessageEnum.PLAYER_WON : MessageEnum.DEALER_WON
      gameOver = true;
      bet = is21 ? bet * 2 * 1.5 : 0
  ...
  const double = async () => {
    ...
    const res = await addUserGameData({
      wallet: address,
      baseScores: state.bet, // 进来已经扣除过
      totalScores: state.bet, // 进来已经扣除过
      isDouble: 1,
      is21: 0,
      ...formatCards(),
      action: 1
      ...
      <div className={classNames(style.gameOver, 'flex flex-col justify-center items-center')}>
        {
          isPlayAgain ? (
            <div className={style.gameStatusContainer}>
              {
                message !== MessageEnum.NULL && <Image className=' mt-[-20px]' src={MAP_MESSAGE_IMG[message]} alt='' width={249} height={66} />
              }
  ...
  return (
    <div className={classNames(style.gameLayout)}>
      ...
      {
        state.gameOver ? renderChipList() : <div className={style.gameContainer}>
      ...
      }
      {
        ...
        >
          <Image src={MAP_MESSAGE_IMG_TOAST[message]} alt='' width={321} height={116} />
        </Mask>
      }
    </div>
  );
